...
[
    ...
    qr#    0: 0x[0-9a-f]+ - (<unknown>|hostcalls\.wasm)!__rust_start_panic#,
    ...
    qr#    8: 0x[0-9a-f]+ - (<unknown>|hostcalls\.wasm)!proxy_wasm::dispatcher::Dispatcher::on_http_request_headers::h[0-9a-f]+#,
    qr#    9: 0x[0-9a-f]+ - (<unknown>|hostcalls\.wasm)!proxy_on_request_headers#,
    qr#(note: using the `WASMTIME_BACKTRACE_DETAILS=1` environment variable may show more debugging information)?#,
    qr#^$#,
    qr#Caused by:#,
...
[
    ...
    qr/custom trap/,
    qr/\[error\] .*? \[wasm\] error while executing at wasm backtrace:/,
    qr#    0: 0x[0-9a-f]+ - .*?(abort|__rust_start_panic)#,
