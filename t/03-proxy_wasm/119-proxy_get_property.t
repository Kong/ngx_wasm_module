...
$ENV{TEST_NGINX__REQ_PROPERTIES} = "
    request.path,
    request.url_path,
    request.host,
    request.scheme,
    ...
    request.size,
    request.total_size";
$ENV{TEST_NGINX__REQ_PROPERTIES} =~ s/[\n\r\s]//g;
...
qq{
    location /t-especial {
        proxy_wasm hostcalls 'on=request_headers \
                              ...
                              name=$ENV{TEST_NGINX__REQ_PROPERTIES}';
        echo ok;
    }
}
--- more_headers
...
my $on_phases = CORE::join(',', @phases);
qq{
    location /t-especial {
        proxy_wasm hostcalls 'on=$on_phases \
                              test=/t/log/properties \
                              name=$ENV{TEST_NGINX__REQ_PROPERTIES}';
        echo ok;
    }
...
foreach my $phase (@phases) {
    foreach my $var (split(',', $ENV{TEST_NGINX__REQ_PROPERTIES})) {
        $checks .= "$var: .+ at $phase\n";
    }
}
qr/$checks/
...
=== TEST 3: proxy_wasm - get_property() - request properties - on_http_call_response
--- load_nginx_modules: ngx_http_echo_module
--- wasm_modules: hostcalls
--- config
    location /dispatched {
...
qq {
    location /t {
        proxy_wasm hostcalls 'on=$on_phases test=/t/log/properties \
...
--- config
    location /t {
        ...
        proxy_wasm hostcalls 'on=response_headers test=/t/log/properties name=upstream.address,upstream.port';
        echo fail;
    }
--- response_body_like
\A\<!doctype html\>.*
...
--- config eval
my @p = (
    "request_headers", "response_headers", "response_body", "response_trailers"
);
my @con_vars = (
    "source.address",
    "source.port",
...
my $vars = CORE::join(',', @con_vars);
my $phases = CORE::join(',', @p);
qq {
...
=== TEST 11: proxy_wasm - get_property() - wasm properties - on: request_headers,request_body,response_headers,response_body,response_trailers
--- load_nginx_modules: ngx_http_echo_module
--- wasm_modules: hostcalls
--- config eval
my @p = (
...
--- config
    location /t {
        proxy_wasm hostcalls 'on=request_headers test=/t/log/properties name=request.path';
        echo ok;
    }
--- request eval
use URI::Escape;
