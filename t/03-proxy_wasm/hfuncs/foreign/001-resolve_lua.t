...
=== TEST 2: proxy_wasm - resolve_lua, bad name
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
=== TEST 3: proxy_wasm - resolve_lua (no yielding)
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
=== TEST 4: proxy_wasm - resolve_lua (yielding)
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
=== TEST 5: proxy_wasm - resolve_lua, IPv6 record
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
=== TEST 6: proxy_wasm - resolve_lua, multiple calls (yielding)
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
[
    qr/resolved \(yielding\) httpbin\.org to \[\d+, \d+, \d+, \d+\]/,
    qr/resolved \(yielding\) example\.com to \[\d+, \d+, \d+, \d+\]/
]
--- no_error_log
[error]
[crit]
...
=== TEST 7: proxy_wasm - resolve_lua, multiple calls (mixed)
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm_lua_resolver on;
...
=== TEST 8: proxy_wasm - resolve_lua, on_tick
--- wasm_modules: hostcalls
--- config
    location /t {
...
=== TEST 9: proxy_wasm - resolve_lua, on_http_call_response
--- wasm_modules: hostcalls
--- config
    location /t {
...
=== TEST 10: proxy_wasm - resolve_lua, on_foreign_function
--- wasm_modules: hostcalls
--- config
    location /t {
        proxy_wasm hostcalls 'test=/t/resolve_lua \
