...
--- skip_eval: 6: ($::osname =~ m/darwin/ && $::archname =~ /arm64/)
--- skip_no_debug: 6
...
=== TEST 4: proxy_wasm queue shm - push when data_size == buffer_size, macOS (arm64)
--- skip_eval: 6: ($::osname !~ m/darwin/ && $::archname !~ /arm64/)
--- skip_no_debug: 6
--- load_nginx_modules: ngx_http_echo_module
--- wasm_modules: hostcalls
...
=== TEST 5: proxy_wasm queue shm - push when data size == buffer_size + 1, macOS (arm64)
--- skip_eval: 6: ($::osname !~ m/darwin/ && $::archname !~ /arm64/)
--- load_nginx_modules: ngx_http_echo_module
--- wasm_modules: hostcalls
--- shm_queue: test 262144
...
=== TEST 6: proxy_wasm queue shm - push when data size == buffer_size - 1, macOS (arm64)
--- skip_eval: 6: ($::osname !~ m/darwin/ && $::archname !~ /arm64/)
--- load_nginx_modules: ngx_http_echo_module
--- wasm_modules: hostcalls
--- shm_queue: test 262144
