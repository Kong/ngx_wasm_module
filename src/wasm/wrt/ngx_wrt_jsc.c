...
static void ngx_jsc_destroy_instance(ngx_wrt_instance_t *instance);


static wasm_engine_t  *jsc_engine = NULL;


static ngx_int_t
ngx_jsc_init_conf(wasm_config_t *config, ngx_wavm_conf_t *conf,
    ngx_log_t *log)
{
...
static ngx_int_t
ngx_jsc_init_engine(ngx_wrt_engine_t *engine, wasm_config_t *config,
    ngx_pool_t *pool, ngx_wrt_err_t *err)
{
    ...
    if (engine->engine == NULL) {
        ...
        if (!jsc_engine) {
            jsc_engine = wasm_engine_new();
            if (jsc_engine == NULL) {
                return NGX_ERROR;
            }
        }

        engine->engine = jsc_engine;
    }
...
static void
ngx_jsc_destroy_engine(ngx_wrt_engine_t *engine)
{
...
static ngx_int_t
ngx_jsc_wat2wasm(wasm_byte_vec_t *wat, wasm_byte_vec_t *wasm,
    ngx_wrt_err_t *err)
{
    wasm_byte_vec_t  out;
...
static ngx_int_t
ngx_jsc_validate(ngx_wrt_engine_t *engine, wasm_byte_vec_t *bytes,
    ngx_wrt_err_t *err)
{
...
static ngx_int_t
ngx_jsc_init_module(ngx_wrt_module_t *module, ngx_wrt_engine_t *engine,
...
static ngx_int_t
ngx_jsc_link_module(ngx_wrt_module_t *module, ngx_array_t *hfuncs,
    ngx_wrt_err_t *err)
{
    size_t                    i, j;
...
static void
ngx_jsc_destroy_module(ngx_wrt_module_t *module)
{
    size_t  i;
...
static ngx_int_t
ngx_jsc_init_store(ngx_wrt_store_t *store, ngx_wrt_engine_t *engine,
    void *data)
{
...
static void
ngx_jsc_destroy_store(ngx_wrt_store_t *store)
{
...
static ngx_int_t
ngx_jsc_init_instance(ngx_wrt_instance_t *instance, ngx_wrt_store_t *store,
    ngx_wrt_module_t *module, ngx_pool_t *pool, ngx_wrt_err_t *err)
{
    size_t               i;
    ngx_uint_t           nimports = 0;
    ngx_wavm_hfunc_t    *import;
    ngx_jsc_hfunc_ctx_t  *hctx = NULL;
    wasm_func_t         *func;
    wasm_extern_vec_t import_vec = {0};
    ...
    instance->ctxs = ngx_pcalloc(pool,
                                 sizeof(ngx_jsc_hfunc_ctx_t) * module->nimports);
    ...
    import_vec.size = nimports;
    import_vec.data = instance->imports;
...
error:
    ...
    ngx_jsc_destroy_instance(instance);
...
static void
ngx_jsc_destroy_instance(ngx_wrt_instance_t *instance)
{
    size_t          i;
    wasm_func_t    *func;
    wasm_extern_t  *ext;
...
static ngx_int_t
ngx_jsc_init_extern(ngx_wrt_extern_t *ext, ngx_wrt_instance_t *instance,
    ngx_uint_t idx)
{
...
static void
ngx_jsc_destroy_extern(ngx_wrt_extern_t *ext)
{
    /* NOP */
    return;
}
...
static ngx_int_t
ngx_jsc_call(ngx_wrt_instance_t *instance, ngx_str_t *func_name,
...
static wasm_trap_t *
ngx_jsc_trap(ngx_wrt_store_t *store, wasm_byte_vec_t *msg)
{
...
static u_char *
ngx_jsc_log_handler(ngx_wrt_res_t *res, u_char *buf, size_t len)
{
    u_char  *p = buf;
...
ngx_wrt_t  ngx_wrt = {
    ngx_jsc_init_conf,
    ngx_jsc_init_engine,
    ngx_jsc_destroy_engine,
    ngx_jsc_validate,
    ngx_jsc_wat2wasm,
    ngx_jsc_init_module,
    ngx_jsc_link_module,
    ngx_jsc_destroy_module,
    ngx_jsc_init_store,
    ngx_jsc_destroy_store,
    ngx_jsc_init_instance,
    ngx_jsc_destroy_instance,
    ngx_jsc_call,
    ngx_jsc_init_extern,
    ngx_jsc_destroy_extern,
    ngx_jsc_trap,
    NULL,                              /* get_ctx */
    ngx_jsc_log_handler,
};
