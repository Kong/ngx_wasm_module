...
typedef enum {
    ...
    NGX_PROXY_WASM_ISOLATION_FULL = 2,
...
typedef enum {
    ...
    NGX_PROXY_WASM_ERR_INSTANCE_RECYCLED = 7,
    NGX_PROXY_WASM_ERR_UNKNOWN = 8,
...
typedef ngx_int_t (*ngx_proxy_wasm_ecode_pt)(ngx_proxy_wasm_err_t ecode);
typedef ngx_proxy_wasm_ctx_t *(*ngx_proxy_wasm_get_context_pt)(
    ngx_proxy_wasm_t *filter, void *data);
typedef void (*ngx_proxy_wasm_free_context_pt)(ngx_proxy_wasm_ctx_t *prctx);


// stream ctx (prctx)
// ngx_proxy_wasm_stream_ctx_s
struct ngx_proxy_wasm_ctx_s {
    ...
    ngx_proxy_wasm_filter_ctx_t       *root_filter_ctx;

    /* swap */

    ngx_uint_t                         ecode;
    ngx_wavm_instance_t               *instance;
    ngx_proxy_wasm_filter_ctx_t       *curr_filter_ctx;
...
struct ngx_proxy_wasm_filter_ctx_s {
    ...
    ngx_proxy_wasm_ctx_t              *stream_ctx;
    ngx_pool_t                        *pool;
    ngx_log_t                         *log;
    ngx_wavm_instance_t               *instance;
...
struct ngx_proxy_wasm_s {
    ...
    ngx_proxy_wasm_ctx_t               root_sctx;
...
/* ngx_proxy_wasm_t */
ngx_int_t ngx_proxy_wasm_filter_init(ngx_proxy_wasm_t *pwmodule);
void ngx_proxy_wasm_filter_destroy(ngx_proxy_wasm_t *filter);

ngx_int_t ngx_proxy_wasm_filter_init_root_instance(ngx_proxy_wasm_t *filter);
ngx_int_t ngx_proxy_wasm_filter_init_instance(ngx_proxy_wasm_ctx_t *prctx);
...
ngx_proxy_wasm_filter_ctx_t *ngx_proxy_wasm_filter_get_ctx(
    ngx_proxy_wasm_t *filter, void *data);
ngx_int_t ngx_proxy_wasm_filter_ctx_err_code(ngx_proxy_wasm_filter_ctx_t *fctx,
    ngx_uint_t ecode);
